...
-- (58 rows)
SELECT (name || ' (' || code || ')') AS name_and_code, region FROM country
WHERE continent = 'Africa';

-- 3. The per capita GNP (i.e. GNP multipled by 1000000 then divided by population) of all countries in the 
-- world sorted from highest to lowest. Recall: GNP is express in units of one million US Dollars 
-- (highest per capita GNP in world: 37459.26)
SELECT (gnp * 1000000 / population) AS per_capita_gnp FROM country
WHERE population > 0
...
WHERE population > 0 and countrycode = 'USA';
...
SELECT name, (population / surfacearea) AS population_density, lifeexpectancy FROM country
...
SELECT (gnp - gnpold) AS gnp_difference, ABS(gnp - gnpold) AS abs_gnp_difference, name FROM country
...
SELECT COUNT(city) AS city_count, countrycode
FROM city
...
SELECT AVG(lifeexpectancy) AS avg_life, MIN(lifeexpectancy) AS min_life, MAX(lifeexpectancy) AS max_life, continent
FROM country
WHERE lifeexpectancy IS NOT NULL
GROUP BY continent
